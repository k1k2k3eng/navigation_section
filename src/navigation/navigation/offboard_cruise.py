...
class OffboardControl(Node):
    ...
    def __init__(self):
        super().__init__('offboard_cruise')
        ...
        self.vehicle_local_position_subscriber = self.create_subscription(
            VehicleLocalPosition, '/fmu/out/vehicle_local_position_v1', self.vehicle_local_position_callback, qos_profile_pub)
        self.vehicle_status_subscriber = self.create_subscription(
            VehicleStatus, '/fmu/out/vehicle_status_v1', self.vehicle_status_callback, qos_profile_pub)
        ...
        if self.current_wp_index < len(self.waypoints):
            ...
            # 如果距离小于判定半径，说明已经到达该点
                if distance < self.acceptance_radius:
                    self.get_logger().info(f"==> 成功到达航点 {self.current_wp_index}: {target}")
                    self.current_wp_index += 1 # 目标切换到下一个点
        else:
            # 如果所有航点都跑完了 (index 越界)
                self.get_logger().info("所有巡航任务完成，准备降落...")
                self.land()
                exit(0) # 退出节点
